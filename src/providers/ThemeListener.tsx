...
import { userColorSchemeAtom } from "@modules/app/states/atoms";
...
export default function ThemeListener() {
  ...
  const handleColorModeChange = useCallback((preferences: Appearance.AppearancePreferences) => {
    setUserColorScheme(preferences.colorScheme);
  }, [setUserColorScheme]);

  useEffect(() => {
    const throttledHandler = throttle(handleColorModeChange, 100, { leading: false, trailing: true });
