...
const templates: Template[] = [
  ...
  },
  {
    ...
    description: "Solliciter une rencontre avec un enseignant ou l'administration",
    recipient: 'admin',
  },
  {
    id: '5',
...
const NewConversationModal: React.FC<NewConversationModalProps> = ({
  ...
  const filteredTemplates = templates.filter(
    (template) => selectedRecipient === 'all' || template.recipient === selectedRecipient
  );
        ...
        <View style={themedStyles.modalContent}>
          ...
          <ScrollView style={themedStyles.templateList}>
            {filteredTemplates.map((template) => (
              <TouchableOpacity key={template.id} onPress={() => onSelectTemplate(template)}>
                <CsCard style={themedStyles.templateCard}>
                  <CsText variant="h3" style={themedStyles.templateTitle}>
                    {template.title}
                  </CsText>
                  <CsText style={themedStyles.templateDescription}>{template.description}</CsText>
