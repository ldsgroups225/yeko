...
const fullDaysOfWeek = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'];
...
const ScheduleScreen: React.FC = () => {
  ...
  const renderDatePicker = () => {
    ...
    const mondayDate = new Date(today.setDate(today.getDate() - currentDay + 1));
        ...
        <View style={themedStyles.daysContainer}>
          {daysOfWeek.map((day, index) => {
            ...
            return (
              <TouchableOpacity
                key={day}
                style={[themedStyles.dateButton, isSelected && themedStyles.selectedDateButton]}
  ...
  const renderCurrentTimeLine = () => {
    ...
    const position = ((totalMinutes - startOfDay) / (endOfDay - startOfDay)) * 100;
  ...
  return (
    <View style={themedStyles.container}>
      {renderDatePicker()}
      <CsText style={themedStyles.dayTitle}>{fullDaysOfWeek[selectedDay - 1]}</CsText>
...
const ScheduleItem: React.FC<{ schedule: Schedule }> = React.memo(({ schedule }) => {
  const themedStyles = useThemedStyles<typeof styles>(styles);
  ...
  return (
    <Animated.View style={[themedStyles.scheduleItem, animatedStyle]}>
      ...
      <CsCard style={themedStyles.scheduleCard}>
        ...
        <CsText style={themedStyles.teacherName}>{schedule.teacherName}</CsText>
        {schedule.room && <CsText style={themedStyles.roomText}>{schedule.room}</CsText>}
      </CsCard>
    </Animated.View>
  );
});
