...
import { calculateAverage, formatDate, formatNote, groupBy, truncateText } from '../utils/index';
...
const NoteScreen: React.FC = () => {
  ...
  const fetchNotes = useCallback(async () => {
    ...
    const mockData: Note[] = [
      ...
      { id: '3', subjectId: '2', subjectName: 'Français', note: 14, date: new Date(2024, 2, 11) },
      { id: '4', subjectId: '2', subjectName: 'Français', note: 13, date: new Date(2024, 2, 2) },
  ...
  const summary: NoteSummary = useMemo(() => {
    if (!notes || notes.length === 0) return { averageNote: 0, bestSubject: '', worstSubject: '' };
  ...
  const renderHeader = () => (
    <View style={themedStyles.header}>
      <CsText style={themedStyles.headerTitle}>Notes et moyennes</CsText>
  ...
  return (
    <View style={themedStyles.container}>
      ...
      <AnimatedFlatList
        ...
        data={groupedNotes}
        renderItem={({ item }) => (
          <SubjectCard title={item.title} average={item.average} notes={item.data} />
        )}
...
const SubjectCard: React.FC<{ title: string; average: number; notes: Note[] }> = React.memo(
  ({ title, average, notes }) => {
    const themedStyles = useThemedStyles<typeof styles>(styles);
...
const NoteItem: React.FC<{ note: Note }> = React.memo(({ note }) => {
  ...
  return (
    <View style={themedStyles.noteItem}>
      ...
      <CsText variant="h3" style={[themedStyles.noteValue, { color: getNoteColor(note.note) }]}>
